...
def _set_args(app_part: dict, app_part_name: str = "") -> None:
	if app_part:
		for key, val in app_part.items():
			os.environ.setdefault(key, val)
	else:
		print(f'"{ENV_FILE_PATH}" file does not contain {app_part_name} variables.')
...
def read_and_set_env() -> None:
	...
	if os.path.exists(ENV_FILE_PATH):
		...
		_set_args(app_part=app, app_part_name="app")
		_set_args(app_part=app_settings, app_part_name="app_settings")
