...
from api.models import Blog, Room, RoomCreator
...
def _create_blogs(username: str, blog_count: int) -> None:
	users: list[int] = User.objects.filter(username__contains=username).values_list(
		"pk", flat=True
	)
	...
	for i in users:
		...
		for _ in range(blog_count):
			...
			blog.append(Blog(user_id=i, content=content))
...
def _create_rooms(username: str, room_count: int) -> None:
	...
	for _ in range(room_count):
		...
		RoomCreator.objects.get_or_create(room_id=room.pk, creator_id=i)
